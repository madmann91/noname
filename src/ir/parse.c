...
static void expect(struct parser* parser, const char* msg) {
    COPY_STR(str, parser->ahead.loc.begin.ptr, parser->ahead.loc.end.ptr)
    log_error(
        parser->lexer.log, &parser->ahead.loc,
...
static struct exp_vec parse_many(struct parser* parser, exp_t (*parse_one)(struct parser*)) {
    ...
    while (
        ...
        push_to_exp_vec(&exps, parse_one(parser));
    return exps;
}
...
static void invalid_var(struct parser* parser, const struct loc* loc, size_t index) {
    log_error(
...
static exp_t parse_var(struct parser* parser) {
    ...
    if (!var) {
        invalid_var(parser, &loc, index);
        return new_untyped_err(parser->mod, &loc);
    }
    return new_var(parser->mod, var->type, index, &loc);
}
...
static exp_t parse_match(struct parser* parser) {
    ...
    struct loc loc = make_loc(parser, begin);
    exp_t exp = new_match(parser->mod, pats.elems, vals.elems, pats.size, arg, &loc);
    free_exp_vec(&vals);
    free_exp_vec(&pats);
    return exp;
}
...
static exp_t parse_paren_exp_or_pat(struct parser* parser, bool is_pat) {
    ...
    switch (parser->ahead.tag) {
        case TOK_ABS: {
            ...
            struct loc loc = make_loc(parser, begin);
            return new_abs(parser->mod, var, body, &loc);
        }
        case TOK_ARROW: {
            ...
            struct loc loc = make_loc(parser, begin);
            return new_arrow(parser->mod, var, codom, &loc);
        }
        case TOK_BOT:
        case TOK_TOP: {
            eat_tok(parser, parser->ahead.tag);
        ...
        case TOK_LIT: {
            ...
            if (parser->ahead.tag == TOK_HEX_INT)
                ...
                expect(parser, "literal value");
            eat_tok(parser, parser->ahead.tag);
            struct loc loc = make_loc(parser, begin);
            return new_lit(parser->mod, type, &lit, &loc);
        }
        case TOK_MATCH:
        ...
        default: {
            ...
            struct loc loc = make_loc(parser, begin);
            return new_app(parser->mod, left, right, &loc);
        }
    }
}
